...
To use MSMatch, raw data files produced by a mass spectrometer must be converted into mzML format (Deutsch 2010) using [Proteowizard's](https://proteowizard.sourceforge.io/) msConvert software (Adusumilli and Mallick 2017). There are specific parameters that must be used during conversion.
...
2.  click the "Add" button and enter search parameters one at a time ([Figure 3, left](#fig04-03)); repeat this process to add more.
...
![Figure 3. Dialogs to identify features of interest by upload (left) or manually by clicking the Add button (right).](assets/fig04-03_FOI_input_dialogs.png "Figure 3. Dialogs to identify features of interest by upload (left) or manually by clicking the Add button (right)."){#fig04-03 width="75%"}

------------------------------------------------------------------------

![Figure 4. Manage the feature identification list (left) interactively by adding, editing, or removing features as needed (right) by selecting a row from the table and clicking the appropriate button.](assets/fig04-04_FOI_list.png "Figure 4. Manage the feature identification list (left) interactively by adding, editing, or removing features as needed (right) by selecting a row from the table and clicking the appropriate button."){#fig04-04 width="100%"}
...
![Figure 5. Click "Process Data" to validate experiment data and choose one of the options that appear or navigate to the desired page using the left-hand navigation menu.](assets/fig04-05_process_data_button.png "Figure 5. Click “Process Data” to validate experiment data and choose one of the options that appear or navigate to the desired page using the left-hand navigation menu."){#fig04-05 width="75%"}
...
![Figure 6. Compound matching options. Select a feature of interest and search type, then click the 'Search' button.](assets/fig04-06_compound_match_screen.png "Figure 6. Compound matching options. Select a feature of interest and search type, then click the 'Search' button."){#fig04-06 width="75%"}
...
![Figure 10. Fragment matching options. Select a feature of interest and click the 'Search' button.](assets/fig04-10_fragment_match_screen.png "Figure 10. Fragment matching options. Select a feature of interest and click the 'Search' button."){#fig04-10 width="75%"}
...
![Figure 11. Graphical display of fragment annotations on an uncertainty mass spectral plot and the measured data provided to MSMatch.](assets/fig04-11_fragment_match_spectra.png "Figure 11. Graphical display of fragment annotations on an uncertainty mass spectral plot and the measured data provided to MSMatch."){#fig04-11 width="100%"}
...
![Figure 13. Additional information available for compounds.](assets/fig04-13_additional_compound_info.png "Figure 13. Additional information available for compounds."){#fig04-13 width="33%"}
...
![Figure 14. Additional information available for peaks.](assets/fig04-14_additional_peak_info.png "Figure 14. Additional information available for peak."){width="50%"}
...
-   Loading the entire project from the compliance script (i.e. MSMatch was launched using `start_app("spectral_match")`) provides additional actions and includes a live database connection with the ability to read data into tables and preserve them for further analysis. Use the function [`close_up_shop()`](#fn_close_up_shop) with the argument `back_up_connected_tbls` set to `TRUE` to preserve these, or the default FALSE to simply close all connections including the API server).
-   If launching the app directly and using the default settings there will be a session object named `plumber_service` connected to that server. To stop it, use the [`api_stop`](#fn_api_stop) function from the console or stop the service directly using `plumber_service\$kill()`; it will also generally stop when the calling R process closes (e.g. when RStudio is closed), but it is highly recommended to stop it manually to prevent hanging connections.
...
[BEN CAN YOU PLEASE ADD SOME TEXT HERE IF NECESSARY AND DELETE THIS SECTION AND THE LINK TO IT IN [STEP 3](#msmatch-step3) IF NOT]

### Compound and Fragment Match Algorithms {#msmatch-algorithms .unnumbered}

All relevant functions for compound and fragment match algorithms are in the `R/spectral_analysis` directory. Currently, the compound search tool of MSMatch can search the database using the precursor ion *m/z* of the unknown compound ([`search_precursor`](#fn_search_precursor){target="_blank"} function) to reduce the processing time and compare only reference mass spectra with the same (defined below) precursor ion *m/z* or compare the mass spectra of the unknown compound to all compounds in the database ([`search_all`](#fn_search_all){target="_blank"} function). The [`search_precursor`](#fn_search_precursor){target="_blank"} function first identifies all peaks that have precursor ion m/z values that are within a range, as dependent on the type of MS2 experiment used:
...
The NIST Mass Spectral Match for Non-Targeted Analysis application provides a new way to make NTA tools developed at NIST using the Database Infrastructure for Mass Spectrometry accessible to both internal and external stakeholders. It is the first demonstration of tools that can be built on top of databases conforming to the [DIMSpec project](#intro-start), which can be repurposed for any class of chemicals or project of interest using mass spectrometry data.
