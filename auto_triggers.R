...
make_sql_trigger <- function(target_table,
  ...
  trigger_comment <- glue::glue("/* [autogenerated] Trigger to control normalization compliance {action_occurs} {trigger_action} actions on table '{target_table}' */")
...
build_triggers <- function(db_map, references_in = "references", create_insert_trigger = TRUE, create_update_trigger = TRUE, save_to_file = NULL, append = TRUE) {
  ...
  if (is.null(save_to_file)) {
    ...
    # TODO readr::write_file(unlist(triggers), save_to_file)
  }
}