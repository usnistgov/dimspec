...
To gather the relevant sample, method, and compound data required for context and feature identification, a macro-enabled Microsoft Excel workbook (Microsoft Office 365) was developed and named the NTA Method Reporting Tool (NTA-MRT). The NTA-MRT is designed to collect data on a single sample (single data file). The NTA-MRT is a tool that provides the ability to collect sample and method-specific information with controlled ontology on specific parameters, such as sample classification, chromatographic column stationary phase chemistry, and mass spectrometric ionization technique. In addition, the NTA-MRT has a "peaks" portion that users can enter identified chemical compounds in the sample and provide a designated level of confidence for that identification (based on the PFAS Confidence of Identification scale described by Charbonnet et al.)[@TODO cite] Finally, the NTA-MRT can generate MS\^2 fragment annotation tables for users to provide elemental formulas and structures (if available) for observed fragments of individual identified chemical compounds. The fragment annotation step is highly recommend, but optional.

Once complete, the NTA-MRT has an export button that converts the completed fields into a schema-defined JSON file, which can be used by subsequent tools in the data collection workflow in conjunction with a converted mzML raw data file.

### Quality evaluation of data and metadata

While the procedure can be run via CLI, the quality evaluation of the data has been streamlined into the MSQC Shiny application (described below). Using both the mzML raw data file and the NTA-MRT-exported JSON file, the data is evaluated.

The quality evaluation includes seven data quality checks ensuring that: (1) the reported precursor ion(s) are within reported mass accuracy bounds given an ideal ion mass for each identified analyte, (2) the MS1 isotopic pattern matches that of a calculated isotopic pattern above an acceptability threshold, (3) precursor ion(s) are present in the provided data file, (4) annotated fragments are present in the provided data file, (5) the mass accuracy of annotated fragments are within reported instrumental error ranges given an ideal ion mass for each identified fragment, (6) the reported elemental formulas for each annotated fragment are a valid subset of the identified analyte, and (7) if there is a SMILES structure provided that it reasonably the elemental formula of the annotated fragment. Additionally, parameters for an optimized uncertainty mass spectrum are calculated for import which facilitates searching of unknown data against recorded fragmentation patterns.
...
To get started setting up the project toolkit, download it from [GitHub](@TODO%20link).
...
[@TODO Ben describe the acquisition and/or generation of the data and incorporation of the data]

## Current database size and characteristics
...
In this paper we present the DIMSpec project, a freely available and open source toolkit for rapid iteration of databases supporting data storage and retrieval for non-targeted analysis of high resolution mass spectrometry data. Included in the project repository is a database of more than 24,000 [@TODO this is probably the number of msdata rows, which is a bit misleading, I would go with the number of Peaks x 2 (MS1 and MS2) to be the spectral count] high quality HRAMS spectra for per- and polyfluorinated alkyl substances. The toolkit and any DIMSpec databases are easily extensible and can be customized for a wide variety of use cases. While DIMSpec is not intended to support quantitative measurements at this time, as data storage is focused specifically around chromatographic peaks it could easily do so by adding a table referencing the peak objects with columns storing desireable quantitative measures such as integrated intensity, calculated mass fraction, and units.
