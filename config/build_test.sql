...
(
	...
	CHECK (local_pos >= 0),
	CHECK (local_neg >= 0),
	FOREIGN KEY (source_type) REFERENCES source_types(id) ON UPDATE CASCADE
);
...
(
	id INTEGER PRIMARY KEY,
	name TEXT UNIQUE 
		/* IUPAC name for mobile phase solvents */
);
...
(
	solvent_id INTEGER, 
		/* foreign key to the solvents table */
	alias TEXT UNIQUE,
		/* human meaningful name(s) associated with a solvent */
	FOREIGN KEY (solvent_id) REFERENCES solvents(id) ON UPDATE CASCADE
);
...
(
	...
	FOREIGN KEY (component) REFERENCES solvents(id) ON UPDATE CASCADE,
...
(
	...
	ms_vendor INTEGER, 
		/* vendor of the mass spectrometer */
	ms_type TEXT, 
		/* type of mass analyzers (QTOF, Q-ORBITRAP, etc.) */
	qc_method INTEGER, 
		/* (0, 1) boolean: does the experiment have a QC method in place */
	qc_type TEXT, 
		/* category of QC analysis (TBD) */
	source TEXT, 
		/* citation for the experimental method */
	FOREIGN KEY (ms_vendor) REFERENCES vendors(id) ON UPDATE CASCADE,
...
(
	...
	sample_class INTEGER, 
		/* foreign key to sample_classes */
	source TEXT, 
		/* citation for the sample source */
	data_generator TEXT, 
		/* generator of data for this sample */
	msconvert_settings INTEGER, 
		/* settings for the msconvert program used to generate data from this sample */
	FOREIGN KEY (sample_class) REFERENCES sample_classes(id) ON UPDATE CASCADE,
	FOREIGN KEY (msconvert_settings) REFERENCES msconvert_settings(id) ON UPDATE CASCADE
);
...
(
	...
	measured_mz1 REAL NOT NULL, 
		/* precursor ion m/z */
	peak_timestamp TEXT, 
		/* timestamp for the peak (YYYY-MM-DD HH:MM:SS UTC) */
	charge INTEGER NOT NULL, 
		/* ion charge state (-1, 1) */
	rt_start REAL NOT NULL, 
		/* peak retention time start point */
	rt_center REAL NOT NULL, 
		/* peak retention time centroid, derived */
	rt_end REAL NOT NULL, 
		/* peak retention time end point */
	FOREIGN KEY (compound_id) REFERENCES compounds(id) ON UPDATE CASCADE,
	FOREIGN KEY (exp_id) REFERENCES methods(id) ON UPDATE CASCADE,
...
(
	...
	scantime REAL NOT NULL, 
		/* scan time of spectrum */
	ms1_data TEXT NOT NULL, 
		/* locator/actual MS1 isotope data */
	contributor TEXT, 
...
(
	...
	scantime REAL NOT NULL, 
		/* scan time of spectrum */
	ms2_data TEXT NOT NULL, 
		/* locator/actual MS2 fragmentation data */
	FOREIGN KEY (peak_id) REFERENCES peaks(id) ON UPDATE CASCADE
);
...
(
	...
	source TEXT NOT NULL DEFAULT 'user', 
		/* citation/source for fragment identity, "user" is an option */
	CHECK (charge IN (-1, 1)),
...
(
	fragment_id INTEGER NOT NULL, 
		/* foreign key to fragment_table */
	data_id INTEGER NOT NULL, 
		/* foreign key to ms1data */
	mz_error REAL NOT NULL, 
		/* measured mass error from fragment_table$MZ, derived */
	FOREIGN KEY (fragment_id) REFERENCES fragments(id) ON UPDATE CASCADE,
	FOREIGN KEY (data_id) REFERENCES ms1data(id) ON UPDATE CASCADE
);
...
CREATE VIEW IF NOT EXISTS element_isotopes AS
	...
	FROM elements e
	LEFT JOIN isotopes i ON e.atomic_number = i.atomic_number;
...
CREATE VIEW IF NOT EXISTS view_msconvert_settings AS SELECT * FROM msconvert_settings;
...
.read config/import_static.sql /*magicsplit*/
