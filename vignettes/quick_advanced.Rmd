...
Further sections (with the exception of Environment Settings) will assume that the `/R/compliance.R` file has been sourced and environments established. Users are referred to the [DIMSpec User Guide](https://pages.nist.gov/dimspec) for full documentation, including function documentation. At any time from a session where the compliance script has been run, documentation is available using (1) `user_guide` to open the full User Guide in a browser, (2) `fn_guide` to launch a browser pointing to HTML function documentation index, or (3) `fn_help(X)` where `X` is the name of a function to see documentation for that function.
...
This section describes in brief some aspects of databases produced by the DIMSpec project; more details are available in the [DIMSpec User Guide](https://pages.nist.gov/dimspec).
...
1. `user_guide` will launch a web browser to the [DIMSpec User Guide](https://pages.nist.gov/dimspec) from the console (either hosted or local).
...
It may be useful at times to flush the logs directory. Use the `flush_dir` function with the logs directory (e.g. `flush_dir(LOG_DIRECTORY)`) to flush the log files. Some additional details are availble in the [Technical Details > Logger section of the DIMSpec User Guide](https://pages.nist.gov/dimspec/docs/technical-details.html#logger).
...
All other settings are left to the individual applications to define needs. See more details in the Shiny Applications section of Technical Details in the [DIMSpec User Guide](https://pages.nist.gov/dimspec/docs/technical-details.html#shiny-applications) or the [DIMSpec Quick Guide - Shiny Web Applications](https://pages.nist.gov/dimspec/docs/docs/quick_guides/quick_apps.pdf) vignette for details of using the included application template.

\newpage

## Final Words

Due to its nature, the DIMSpec project includes several aspects, some complicated. The quick guides are meant as a, hopefully, brief reference for some of the more common use cases and settings, and should be viewed as supplements for the full [DIMSpec User Guide](https://pages.nist.gov/dimspec). A note on network hosting: if your organization (as it should) scans for active sites and vulnerabilities, and you are running the plumber API or shiny applications on your local network in the manner described here, contact your IT professionals to ensure adherence to your organization's IT security policies.
